...
def test_health_calls_get_health(client: MoorchehApiClient) -> None:
    ...
    with patch("moorcheh.api.requests.get", return_value=_mock_response(ok=True, status_code=200, json_data=payload)) as get:
        assert client.health() == payload
    get.assert_called_once_with("http://localhost:8080/health", timeout=15)
...
def test_list_namespaces(client: MoorchehApiClient) -> None:
    ...
    get.assert_called_once_with("http://localhost:8080/namespaces", timeout=15)
...
def test_delete_namespace_job_status(client: MoorchehApiClient) -> None:
    ...
    get.assert_called_once_with("http://localhost:8080/namespaces/docs/delete-jobs/job-del-1", timeout=15)
...
def test_upload_job_status(client: MoorchehApiClient) -> None:
    ...
    get.assert_called_once_with("http://localhost:8080/namespaces/docs/upload-jobs/job-up-1", timeout=15)
